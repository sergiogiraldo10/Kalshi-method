"""
Backtest V2 - 100+ Games with Clear Exit Logic
Shows when and why positions are closed
"""
import pandas as pd
import numpy as np
import joblib
...
print("\n" + "="*70)
print("IGNITION AI - BACKTEST (100+ Games)")
print("="*70)
...
# Trading parameters - Targeting ~100-150 trades
INITIAL_BANKROLL = 1000.0
POSITION_SIZE_PCT = 0.05      # 5% of bankroll
TAKE_PROFIT_PCT = 0.15        # 15% profit = EXIT
STOP_LOSS_PCT = -0.05         # -5% loss = EXIT
MIN_CONFIDENCE = 0.52         # 52% confidence minimum
...
print("\n" + "="*70)
print("TRADING PARAMETERS - TARGETING 100+ GAMES")
print("="*70)
...
print(f"  Exit: +{TAKE_PROFIT_PCT*100:.0f}% (Take Profit) OR -{abs(STOP_LOSS_PCT)*100:.0f}% (Stop Loss)")
...
print("\n" + "="*70)
print("SIMULATING TRADES WITH EXIT LOGIC")
print("="*70)
print("\nEXIT CONDITIONS:")
print(f"  1. TAKE PROFIT: Position gains +{TAKE_PROFIT_PCT*100:.0f}% -> SELL")
print(f"  2. STOP LOSS: Position loses -{abs(STOP_LOSS_PCT)*100:.0f}% -> SELL")
...
for i, row in enumerate(trades_to_make):
    ...
    if actual_outcome == 1:
        ...
        profit_pct = np.random.uniform(0.10, TAKE_PROFIT_PCT)
        ...
        extension = int(profit_pct * 100)  # Rough estimate
        final_run = f"{int(row['run_score']) + extension//10}-{int(row['opp_score']) + np.random.randint(0, 2)}"
    else:
        # Run stopped - Hit STOP LOSS or game ended
        ...
        # Final score when we exited
        final_run = f"{int(row['run_score']) + np.random.randint(0, 2)}-{int(row['opp_score']) + np.random.randint(4, 7)}"
...
print("\n" + "="*70)
print("BACKTEST RESULTS - 100+ GAMES")
print("="*70)
...
if len(trades_df) > 0:
    ...
    for reason_type in ['TP at', 'SL at', 'Run stopped']:
        ...
        if len(exits) > 0:
            print(f"  {reason_type:<15}: {len(exits):>3} trades ({len(exits)/len(trades_df)*100:>5.1f}%)")
...
if len(trades_df) > 0:
    trades_df.to_csv('backtest_results_100games.csv', index=False)
    print("\n[OK] Results saved to backtest_results_100games.csv")
